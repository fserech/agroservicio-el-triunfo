...
export class VentasDetailComponent implements OnInit {
  ...
  venta:      any     = null;
  load              = true;
  showPrintModal    = false;   // controla visibilidad del modal
  ...
  openPrintModal():  void { this.showPrintModal = true; }
  closePrintModal(): void { this.showPrintModal = false; }
...
<html lang="es"><head>
  ...
  ${v.cliente_nit ? `<div class="row"><span>NIT:</span><span>${v.cliente_nit}</span></div>` : ''}
  <div class="row"><span>Método pago:</span><span>${metodoPagoLabel[v.metodo_pago as string] ?? v.metodo_pago ?? ''}</span></div>
  <div class="line"></div>
  <table>
    <thead><tr><th style="text-align:left">Producto</th><th style="text-align:center">Cant</th><th style="text-align:right">P/U</th><th style="text-align:right">Sub</th></tr></thead>
    <tbody>${rows}</tbody>
  </table>
  <div class="line"></div>
  <div class="row"><span>Subtotal</span><span>${fmt(subtotal)}</span></div>
  ${descuento > 0 ? `<div class="row discount"><span>Descuento</span><span>− ${fmt(descuento)}</span></div>` : ''}
  ${iva > 0 ? `<div class="row"><span>IVA (12%)</span><span>${fmt(iva)}</span></div>` : ''}
  <div class="line"></div>
