...
import {
  matAddOutline, matModeEditOutline, matDeleteOutline,
...
export class UsuariosListComponent implements OnInit {
  ...
  edit(id: number, e: Event): void {
    e.stopPropagation();
    this.router.navigate(['/usuarios', id, 'editar']);
  }

  delete(u: Usuario, e: Event): void {
    e.stopPropagation();
    ...
    this.svc.delete(u.id).subscribe({
      next: () => {
        this.toast.success(`Usuario "${u.nombre}" eliminado`);
        this.items = this.items.filter(x => x.id !== u.id);
        this.deletingId = null;
      },
      error: (err: any) => {
        this.deletingId = null;
        this.toast.error(err?.error?.error || 'No se pudo eliminar el usuario');
      }
    });
  }
}