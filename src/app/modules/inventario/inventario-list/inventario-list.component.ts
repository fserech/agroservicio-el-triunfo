...
@Component({
  selector: 'app-inventario-list', standalone: true,
  ...
  templateUrl: './inventario-list.component.html', styleUrls: ['./inventario-list.component.scss']
})
export class InventarioListComponent implements OnInit {
  ...
  items: InventarioItem[] = []; filtered: InventarioItem[] = [];
  load = false; searchText = '';
  ...
  onEstadoChange(val: string): void { this.estadoFilter = (val as any) || undefined; this.loadData(); }
  applyFilter(): void {
    const s = this.searchText.toLowerCase();
    this.filtered = !s ? this.items : this.items.filter(i => i.nombre?.toLowerCase().includes(s));
  }
  getEstadoLabel(e: string): string { return ({critico:'Crítico',sin_stock:'Sin Stock',bajo:'Bajo',normal:'Normal'} as any)[e] || e; }
  openAjusteFor(id: number, tipo: 'entrada'|'salida'): void {
    ...
    this.svc.ajuste({ producto_id: id, tipo, cantidad: +cantStr, motivo: `Ajuste manual ${tipo}` }).subscribe({
  ...
  verMovimientosDe(id: number): void { this.router.navigate(['/inventario'], { queryParams: { producto: id } }); }
}